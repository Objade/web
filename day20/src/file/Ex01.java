...
public class Ex01 {	
	public static void main(String[] args) throws IOException {
		// 사용자에게 입력을 허용하면, 다양한 입출력 관련 예외가 발생할 수 있음
...
//		System.out.println("입력한 글자 : " + ch);
		...
		File f1 = new File("a.txt");					// 파일 이름만 문자열로 전달 (상대경로) -> 해당 프로젝트에 파일이 저장됨
		File f2 = new File("C:\\20221110");				// 폴더 및 파일의 절대 경로를 문자열로 전달
		File f3 = new File(f2, "b.txt");				// 부모 경로를 파일로, 파일 이름은 문자열로 전달
		File f4 = new File("C:\\20221110", "c.txt");	// 부모 경로를 문자열로, 파일 이름은 문자열로 전달
		
		
		// 자바에서 위치와 이름을 지정하여 파일 객체를 생성해도, 실제 파일이 만들어지는 것은 아님
		
		// 자바에서 파일 객체를 생성하는 작업은 실제 파일의 존재와 상관이 없음
		
		f1.createNewFile();		// 비어있는 새로운 파일을 생성함 (생성 여부를 boolean으로 반환)
		
		f2.mkdir();			// 폴더(디렉토리)를 생성함 (생성여부를 boolean으로 반환)
