...
class PowerChargeCable extends Object implements USB {	// 전력 전송	
	// 슈퍼클래스가 지정되어 있어도 상관없음 (다중상속 가능)
	
	@Override	// action은 추상메서드이기 때문에 강제로 오버라이딩 해서 내용을 구성해야함.
	public void action() {
...
class PC {
	// 컴퓨터에 외부 장치를 연결하기 위해서는 USB 인터페이스 규격을 만족하면 됨
	// -> 자료형이 USB이면 가능함 (다형성)
...
public class Ex04 {
	public static void main(String[] args) {
		// 서로 다른 자료형의 3가지 장치가 모두 하나의 함수에 의해 컴퓨터와 연결될 수 있음
		...
		// 함수형 인터페이스란, 단 하나의 추상메서드만 가지는 인터페이스를 말함
		// 함수형 인터페이스는 람다식에 의해 객체를 생성할 수 있음
		// 익명 클래스에 의한 객체 생성보다 식이 간편하고, 내부 작동도 다름
		
		USB mouse = () -> System.out.println("마우스 연결됨...");
		pc.connect(mouse);
		
		// 미구현된 함수의 내용만 적어주면 객체를 생성할 수 있음
		
		// 람다식 구조
		// (매개변수) -> { 실행할 내용 };
		// 단, 이때 실행할 내용이 한줄이면 중괄호 생략 가능.
